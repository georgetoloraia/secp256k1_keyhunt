...
//     // generate 256-bit binary with exactly 128 zeros and 128 ones
//         std::string bits(128, '0');
//         bits += std::string(128, '1');
//         std::random_device rd;
//         std::mt19937 g(rd());
//         std::shuffle(bits.begin(), bits.end(), g);

//         // Debug print
//         std::cout << "[DEBUG] Random 256-bit pattern: " << bits << "\n";
//         // std::cout << "[DEBUG] As decimal: " << bits.get_str(10) << "\n";
//         // std::cout << "[DEBUG] As hex     : " << bits.get_str(16) << "\n";
...
//     while (true) {
//         // mpz_class r = mpz_class(1 + rand() % 12);
...
void process_forever() {
    while (true) {
    // generate 256-bit binary with exactly 127 zeros and 129 ones
        std::string bits(127, '0');
        bits += std::string(129, '1');
        std::random_device rd;
        std::mt19937 g(rd());
        std::shuffle(bits.begin(), bits.end(), g);
        ...
        for (const auto& delta : pr_values) {
            mpz_class priv_key = (r + delta) % max_key;
            if (priv_key <= 0 || priv_key >= max_key) continue;
